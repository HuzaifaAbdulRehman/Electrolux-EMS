...
mysqldump -u root -p"REDACTED" electricity_ems > backup_before_cleanup.sql
```
...
mysql -u root -p"REDACTED" electricity_ems < PRE_SUBMISSION_CLEANUP.sql > cleanup_results.txt 2>&1
```
...
mysql -u root -p"REDACTED" -D electricity_ems -e "
...
```bash
# Restore from backup
mysql -u root -p"REDACTED" electricity_ems < backup_before_cleanup.sql
